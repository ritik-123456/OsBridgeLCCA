...
class ComponentWidget(QWidget):
    def __init__(self, parent_widget, event_filter):
        ...
        self.data = construction_materials.get(KEY_SUBSTRUCTURE) # Using SubStructure key
        self.material_rows = []
    ...
    def add_row_from_popup_data(self, data):
        ...
        if self.parent_widget and getattr(self.parent_widget, 'data_manager', None) and self.parent_widget.project_id:
            try:
                ...
                sub_cat ="sub_structure"
                
                # Call the manager
                # NOTE: We assume everything in SubStructure widget goes under "construction_work_data"
...
class SubStructure(QWidget):
    ...
    def set_project_id(self, pid):
        self.project_id = pid
        print(f"SubStructure Widget: Project ID set to {pid}")
    ...
    def save_data(self):
        ...
        if self.data_id:
            self.database_manager.replace_structure_work_rows(KEY_SUBSTRUCTURE, data, self.data_id)
        else:
    ...
    #Ritik - START: Improved Excel Data Mapping for SubStructure Widget
    def load_from_excel_sections(self, sections_data):
        """
        Load parsed Excel data sections into the SubStructure widget.
        Data is grouped by component type - all materials for the same component
        are displayed under one component widget.
        """
        print(f"\n[EXCEL IMPORT] Loading {len(sections_data)} section(s) into SubStructure widget")
    ...
    #Ritik - END: Improved Excel Data Mapping for SubStructure Widget